...
export async function GET(request) {
  ...
  try {
    ...
    const minutosActual = ahoraMx.getMinutes()
    const tiempoMinutosActual = horaActual * 60 + minutosActual
    ...
    for (const cita of (citas || [])) {
      ...
      const [horaCitaStr, minCitaStr] = cita.hora.split(':')
      const tiempoMinutosCita = parseInt(horaCitaStr) * 60 + parseInt(minCitaStr)
      const diffMinutos = tiempoMinutosCita - tiempoMinutosActual

      // Si la cita es en menos de 2.5 horas (150 min) y más de 30 min, evaluamos enviar recordatorio
      if (diffMinutos > 30 && diffMinutos <= 150) {
        
        // Verificar si ya se envió el recordatorio consultando la tabla mensajes
        ...
        const msj = `⏰ ¡Hola ${nombre}! Te escribimos de Total English para recordarte que tu cita es hoy a las ${cita.hora}. ¡Te esperamos! 😊`
        ...
        try {
          ...
          resultados.enviados++
          console.log(`✅ Recordatorio enviado a ${telefono} vía ${canal}`)
        } catch (e) {
